...
class Bank:
    ...
    def transfer(self, account1: int, account2: int, money: int) -> bool:
        if self.is_valid_account(account1) and self.is_valid_account(account2) and self.does_not_overdraft(account1, money):
    ...
    def withdraw(self, account: int, money: int) -> bool:
        if self.is_valid_account(account) and self.does_not_overdraft(account, money):
            self._balance[account - 1] -= money
            return True
        return False

    def does_not_overdraft(self, account: int, money: int) -> bool:
        return self._balance[account - 1] >= money
